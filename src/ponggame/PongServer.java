...
public class PongServer extends JFrame implements KeyListener, Runnable, WindowListener{
	...
	// - Frame - //
	private static final String TITLE  = "Pong Adíka a Budíka";	
	...
	private int barR      = 20;		// - Player bar width - //
	private int playerH   = 90; 	// - Player bar height - //
   	 ...
   	 try {
   		 serverSoc = new ServerSocket(portAdd);
        	 ...
        	 if(clientSoc.isConnected()){ // - If connected a player start to loop - //
                        //while(!playerC.isReady() || !playerS.isReady()){}
	...
	private Image createImage(){
	    ...
	    // - Score - //
	    g.setColor(new Color(167,169,172));
	    g.setFont(sFont);
	    g.drawString(""+playerS.getScoreS(), WIDTH/2-80, 120);
	    g.drawString(""+playerS.getScoreP(), WIDTH/2+35, 120);
	    
	    // - Player Names - //
	    g.setFont(nFont);
	    g.setColor(new Color(37,206,255));
	    g.drawString(playerS.getName(),WIDTH/10,HEIGHT-20);
	    g.drawString(playerC.getName(),600,HEIGHT-20);
	    
	    
	    // - Player's Bar - //
	    g.setColor(new Color(0,167,216));
	    g.fillRect(playerS.getX(), playerS.getY(), barR, playerH);
	    g.setColor(new Color(0,167,216));
	    g.fillRect(playerC.getX(), playerC.getY(), barR, playerH);
	    
	    // - Ball - //
	    g.setColor(new Color(38,102,186));
	    g.fillOval(playerS.getBallx(), playerS.getBally(), 30, 30);
	...
	public void keyPressed(KeyEvent arg0) {
		...
		if(Restart == true || keycode == KeyEvent.VK_R){
			restartON = true;
